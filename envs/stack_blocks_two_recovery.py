"""
Stack Blocks Two - Recovery Data Generation Variant
...
class stack_blocks_two_recovery(RecoveryMixin, stack_blocks_two):
    """
    Stack two blocks task with funnel-based recovery trajectory generation.

    Extends stack_blocks_two with recovery capabilities from RecoveryMixin.
    """
    ...
    def play_once(self):
        ...
        # Return to home
        self.start_subtask("return_home", arm_tag=str(arm_tag2))
        instruction = self._generate_instruction("return_home")
        self.move(self.back_to_origin(arm_tag=ArmTag(arm_tag2)))
